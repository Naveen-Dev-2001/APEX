...
const ItemMasterModal = ({ mode, rowData, onClose, onSave }) => {
    ...
    useEffect(() => {
        if (isEdit && rowData) {
            setForm({
                ...
                name: rowData.name ?? '',
                product_line_id: rowData.product_line_id ?? '',
                gl_group: rowData.gl_group ?? '',
            });
        } else {
            setForm(EMPTY_FORM);
        }
    ...
    const handleSave = () => {
        const newErrors = {};
        if (!form.item_id) newErrors.item_id = 'Item Id is required';
                ...
                <div className="px-6 py-6 flex flex-col gap-5">
                    <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                        ...
                        <FormField
                            ...
                            onChange={handleChange('product_line_id')}
                        />
                        <FormField
                            label="Gl Group"
                            id="gl_group"
                            value={form.gl_group}
                            onChange={handleChange('gl_group')}
                        />
                    </div>
                </div>
