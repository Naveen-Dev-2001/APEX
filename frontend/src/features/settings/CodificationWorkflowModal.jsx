...
const CodificationWorkflowModal = ({ mode, rowData, onClose }) => {
    ...
    const handleSave = async () => {
        // Validation
        if (!form.lob || !form.department_id) {
            toast.error('LOB and Department ID are required');
            return;
        }
        ...
        // Ensure all approver fields are lists and any empty ones are null for the backend
        ...
        try {
            if (isEdit) {
                await updateCodificationWorkflow(rowData.id, payload);
            ...
            <div className="bg-white rounded-[12px] shadow-2xl w-full max-w-[650px] mx-4 flex flex-col max-h-[92vh]">
                ...
                <div className="flex items-center justify-between px-8 py-6 border-b border-gray-100">
                    <h2 className="text-[19px] font-bold text-[#1a2235]">
                        {isEdit ? 'Edit Config Workflow' : 'Add Config Workflow'}
                    </h2>
                    <button
                        onClick={onClose}
                ...
                <div className="overflow-y-auto px-10 py-8 flex flex-col gap-8">
                    {/* LOB and Dept Selection */}
                    <div className="grid grid-cols-2 gap-6">
                        <Dropdown
                            label="Line of Business (LOB) *"
                            value={form.lob}
                            options={lobsList}
                            onChange={(val) => setForm(prev => ({ ...prev, lob: val }))}
...
export default CodificationWorkflowModal;
