...
const SageGLTab = () => {
  ...
  const openUploadModal = () => {
    setUploadBank(selectedBank !== 'all' ? selectedBank : '');
    setUploadFile(null);
  ...
  const handleUpload = async (fileToUpload) => {
    const targetFile = fileToUpload && fileToUpload.name ? fileToUpload : uploadFile;
    if (!targetFile) {
      toast.error('Please select a file to upload');
      return;
    }

    const selectedUploadRow = (bankAccounts || []).find(
      (row) => String(row?.account_number || '').trim() === String(uploadBank || selectedBank || '').trim(),
    );

    const accountNumber = selectedUploadRow?.gl_account
      || selectedUploadRow?.account_number
      || selectedBankGlAccount
      || selectedBankAccountNumber
      || null;

    const bank = selectedUploadRow?.bank_id
      || selectedUploadRow?.bank_name
      || selectedBankId
      || selectedBankName
      || null;
  ...
  return (
    <div className="flex flex-col space-y-6 h-full min-h-0 overflow-hidden">
      ...
      <div className="flex items-center justify-between bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-medium text-sm transition-colors cursor-pointer"
          >
      ...
      {/* Delete Bank Transactions Confirmation Modal */}
      {deleteTarget && (
        <div className="fixed inset-0 z-[2200] bg-black/40 backdrop-blur-[1px] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-red-100 shadow-xl max-w-md w-full overflow-hidden p-6 text-center animate-in fade-in zoom-in-95 duration-150">
            ...
            <p className="text-xs text-gray-600 mb-6 leading-relaxed">
              Are you sure you want to delete Sage GL transactions for <span className="font-semibold text-gray-800">{deleteTarget}</span>?
            </p>
