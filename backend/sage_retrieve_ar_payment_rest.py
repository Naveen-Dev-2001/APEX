"""
Retrieve Accounts Receivable Payment records from Sage Intacct sandbox using OAuth2 + REST API.
Specifically configured for Accounts Receivable -- Payment.

Endpoint: GET https://api.intacct.com/ia/api/v1/objects/accounts-receivable/payment/{key}

Usage:
  python backend/sage_retrieve_ar_payment_rest.py [entity_id] [max_keys]
...
# AR Payment object path
AR_PAYMENT_OBJECT = "accounts-receivable/payment"

TARGET_ENTITY_ID = os.environ.get("SAGE_ENTITY_ID", "404")
...
# Fields to retrieve during the initial key query
...
def _extract_entity_id(row: dict) -> Optional[str]:
    ...
    for key in ("financialEntity", "financialEntityId", "locationId", "entityId"):
...
async def fetch_payment_keys(access_token: str, entity_id: str) -> List[dict]:
    """
    Use the core query service to list all AR payment keys and summary fields.
    Falls back to an empty list if the query service is unavailable.
    """
    headers = build_entity_headers(access_token, entity_id)
    object_name = AR_PAYMENT_OBJECT
...
async def fetch_payment_list_direct(access_token: str, entity_id: str) -> List[dict]:
    """
    Retrieve list directly from GET /objects/accounts-receivable/payment
    Used as a fallback when the core query service returns no results.
    """
    headers = build_entity_headers(access_token, entity_id)
    url = f"{BASE_URL}/objects/{AR_PAYMENT_OBJECT}"

    async with httpx.AsyncClient(timeout=60) as client:
        print(
            f"[{AR_PAYMENT_OBJECT}] Listing records from direct GET endpoint (Entity: {entity_id})..."
        )
        ...
        total_count = int(meta.get("totalCount", 0) or 0)
        print(
            f"[{AR_PAYMENT_OBJECT}] Direct endpoint retrieved "
...
async def fetch_payment_details(access_token: str, key_rows: List[dict], entity_id: str) -> List[dict]:
    """
    Fetch full AR payment details via:
      GET /objects/accounts-receivable/payment/{key}
    Runs up to 5 concurrent requests with automatic rate-limit back-off.
    """
    headers = build_entity_headers(access_token, entity_id)
    ...
    print(
        f"[{AR_PAYMENT_OBJECT}] Fetching details via "
        f"GET /objects/{AR_PAYMENT_OBJECT}/{{key}} for {len(keys)} keys (Entity: {entity_id})..."
    )
    details: List[dict] = []
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        async def fetch_one(one_key: str) -> Optional[dict]:
            url = f"{BASE_URL}/objects/{AR_PAYMENT_OBJECT}/{one_key}"
        ...
        for idx, row in enumerate(await asyncio.gather(*tasks), start=1):
            ...
            if idx % 50 == 0 or idx == len(keys):
                print(f"[{AR_PAYMENT_OBJECT}] Fetched detail {idx}/{len(keys)}")

    return details


async def process_ar_payment_retrieval(token: str, entity_id: str) -> Dict[str, Any]:
    print(f"\n--- Starting retrieval for object: {AR_PAYMENT_OBJECT} (Entity: {entity_id}) ---")

    key_rows = await fetch_payment_keys(token, entity_id)

    if key_rows:
        details = await fetch_payment_details(token, key_rows, entity_id)
    else:
        # Fallback to direct GET list endpoint
        direct_rows = await fetch_payment_list_direct(token, entity_id)
    ...
    print(f"Success! Retrieved {len(filtered_records)} records for '{AR_PAYMENT_OBJECT}' (Entity: {entity_id}).")
...
async def main() -> None:
    ...
    print("Starting Sage Intacct AR Payment retrieval script via REST API...")
    print(f"Target Entity ID: {entity_id}")
    print(f"REST Endpoint Object: {AR_PAYMENT_OBJECT}")
    print(f"Max Keys Cap: {MAX_KEYS if MAX_KEYS is not None else 'Unlimited (All Records)'}")

    token = await get_oauth_token()
    await process_ar_payment_retrieval(token, entity_id)
