...
def ensure_local_file(file_path: str) -> str:
    ...
    from app.services.azure_blob import get_blob_name_from_path, download_blob_to_file
...
def move_invoice_file(current_path: str, target_category: str) -> Optional[str]:
    ...
    try:
        from app.services.azure_blob import get_blob_name_from_path, container_client, upload_file_to_blob
        ...
        if not source_blob_client.exists():
            for folder_prefix in ["in_progress_files", "deleted_files", "posted_to_sage_files", "archived_files", "non_invoice", "read", "unread"]:
                for prefix in [f"sage/{folder_prefix}", folder_prefix]:
                    candidate_blob = f"{prefix}/{filename}"
