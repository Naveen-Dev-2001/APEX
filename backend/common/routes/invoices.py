...
from common.services.line_grouping import aggregate_items
from common.models.invoice import InvoiceCreate, InvoiceResponse, InvoiceStatus, InvoiceUpdate, InvoicePaginatedResponse
from common.models.workflow import WorkflowStepType, WorkflowStepStatus
from common.database.database import get_db, SessionLocal
...
from common.services.file_manager import init_upload_folders, move_invoice_file, find_file_in_any_folder, get_folder_path
init_upload_folders()
from common.routes.approval_new import StepType
...
from common.database.db_utils import (
...
async def check_duplicate_invoice_endpoint(
    ...
    from common.utils.invoice_registry import check_registry_duplicate
    from common.ai.duplicate_detector import check_duplicate_invoice
...
async def upload_invoices(
    ...
    from common.ai.duplicate_detector import (
...
async def get_invoice_filter_options(
    ...
    if column == "status":
        ...
        from common.database.db_utils import get_status_label
...
async def list_deleted_invoices(
    ...
    if is_approver and not is_admin and not is_finance:
        from common.models.db_models import Delegation
        curr_time = get_ist_now()
        active_delegations = db.query(Delegation.delegator_email).filter(
            Delegation.substitute_email.ilike(current_user.email),
            Delegation.entity == entity,
...
async def debug_last_approved(db: Session = Depends(get_db)):
    from common.models.db_models import Invoice
...
async def update_invoice_status(
    ...
    from common.routes.workflow import (
    ...
    if status == InvoiceStatusEnum.APPROVED:
        ...
        if not any(a.approver_email == current_user.email for a in invoice.approved_by_list):
            from common.models.db_models import InvoiceApprovedBy
...
async def update_invoice(
    ...
    if extracted_data:
        ...
        if new_vendor_id and vendor_changed:
            from common.ai.normalizer import normalize_vendor, normalize_address
    ...
    if "status" in update_data and update_data["status"] == InvoiceStatusEnum.WAITING_APPROVAL:
         from common.routes.workflow import get_vendor_data_from_invoice, get_required_approver_count, get_invoice_total_from_invoice
         from common.models.db_models import User
         from common.repository.repositories import invoice_assigned_approver_repo
    ...
    if "status" in update_data and update_data["status"] == InvoiceStatusEnum.WAITING_CODING:
        def run_auto_coding(inv_id):
            from common.middleware.logger import current_user_var
            ...
            bg_db = SessionLocal()
            try:
                from common.routes.coding import apply_coding_suggestions_to_invoice
    ...
    if "extracted_data" in update_data:
        ...
        def run_coding_sync(inv_id, data_dict, v_name, v_id):
            ...
            bg_db = SessionLocal()
            try:
                from common.routes.coding import update_coding_history
                from common.models.coding import LineItemCoding
                from common.models.db_models import Coding as DBCoding
    ...
    if new_vendor_id != current_vendor_id or new_invoice_number != current_invoice_number:
        from common.utils.invoice_registry import remove_from_registry, register_invoice
...
#     from common.utils.invoice_registry import remove_from_registry
#     remove_from_registry(db, invoice_id)
...
async def delegate_invoice(
    ...
    from common.routes.approval_new import _record_step
    _record_step(
        db, invoice_id,
