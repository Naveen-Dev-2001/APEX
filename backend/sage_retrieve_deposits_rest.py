"""
Retrieve Deposit records from Sage Intacct sandbox using OAuth2 + REST API.
Specifically configured for Cash Management -- Deposit.

Usage:
  python backend/sage_retrieve_deposits_rest.py [entity_id] [max_keys]
...
TARGET_ENTITY_ID = os.environ.get("SAGE_ENTITY_ID", "302")
SAGE_OBJECT = os.environ.get("SAGE_OBJECT", "cash-management/deposit")
...
def _is_target_entity(row: dict, target: Optional[str]) -> bool:
    if not target or target.lower() in ("toplevel", "none", "top"):
        return True  # Include all for top-level/no-filter
    entity_value = _extract_entity_id(row)
    if not entity_value:
        return True  # Include if no explicit entity tag is found (so we don't accidentally drop it)
...
async def fetch_object_keys(access_token: str, object_name: str, entity_id: Optional[str]) -> List[dict]:
...
async def fetch_deposits_direct(access_token: str, entity_id: Optional[str]) -> List[dict]:
    """
    Retrieve list directly from GET /objects/cash-management/deposit
    """
    headers = build_entity_headers(access_token, entity_id)
    url = f"{BASE_URL}/objects/cash-management/deposit"
    
    async with httpx.AsyncClient(timeout=60, verify=VERIFY_SSL) as client:
        scope_desc = f"Entity: {entity_id}" if entity_id else "Top Level"
        print(f"[cash-management/deposit] Listing records from direct GET endpoint ({scope_desc})...")
        ...
        total_count = int(meta.get("totalCount", 0) or 0)
        print(
            "[cash-management/deposit] Direct endpoint retrieved "
...
async def fetch_deposit_details(access_token: str, object_name: str, key_rows: List[dict], entity_id: Optional[str]) -> List[dict]:
    headers = build_entity_headers(access_token, entity_id)
...
async def process_deposit_retrieval(token: str, object_name: str, entity_id: Optional[str]) -> Dict[str, Any]:
    scope_desc = f"Entity: {entity_id}" if entity_id else "Top Level"
    ...
    if key_rows:
        details = await fetch_deposit_details(token, object_name, key_rows, entity_id)
    else:
        # Fallback directly to GET list
        direct_rows = await fetch_deposits_direct(token, entity_id)
    ...
    print(f"Success! Retrieved {len(filtered_records)} records for '{object_name}' ({scope_desc}).")
...
async def main() -> None:
    global MAX_KEYS
    entity_id: Optional[str] = TARGET_ENTITY_ID
    ...
    print(f"Starting Sage Intacct Deposits retrieval script via REST API...")
    ...
    await process_deposit_retrieval(token, SAGE_OBJECT, entity_id)
